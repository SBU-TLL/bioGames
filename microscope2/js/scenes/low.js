...
function lowAdjustLenses() {
    textSetup("Rotate the lenses to lowest objective (4X)", "15%", "35%");
...
function lowDLight() {
    ...
    textSetup("Open the aperture slightly to allow more light to illuminate your slide by clicking the lever and dragging left", "60%", "65%");
    var id = "#diaphragm";
...
function lowAdjustCoarse() {
    ...
    textSetup("Adjust the stage by clicking on the course focus knob and dragging the stage until your sample is in focus.", "60%", "64%");
    var id = "#knobsCoarse";
    if (lowCoarse.isActive()) {
        var clonedComp = highlightComponent(id);
        var handler = function () {
            subHandler(ms.slideBlur, -2, -1, lowCoarse, id, null);
        }
        $(document).bind("mousemove", handler);
    }
}
